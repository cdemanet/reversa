...
export class Writer {
  ...
  // Installs an agent's skills for an engine
  ...
  // Installs the entry file of an engine (CLAUDE.md, AGENTS.md, etc.)
  // force=true: silently overwrites (used by update on untouched files)
  async installEntryFile(engine, { force = false, outputFolder, forwardFolder } = {}) {
    ...
    // File already exists — ask user (merge or skip only)
  ...
  createReversaDir(answers, version) {
    ...
    const config = configTemplate
      ...
      .replace('chat_language = "en"', `chat_language = "${answers.chat_language}"`)
      .replace('doc_language = "en"', `doc_language = "${answers.doc_language}"`)
  ...
  // Copies body templates, scripts, hooks.yml and setup.json to .reversa/
  ...
  _installForwardAssets(reversaDir, answers, version) {
    ...
    // setup.json → .reversa/setup.json (with placeholders)
  ...
  // skipping files that the user has modified. setup.json is always preserved because it
  // carries project-specific data (project-name, installed-at, user prefix format).
  //
  // modifiedSet: Set<string> with relative paths to projectRoot that should NOT be overwritten.
  ...
  // Saves the list of created files in state.json
  saveCreatedFiles() {
