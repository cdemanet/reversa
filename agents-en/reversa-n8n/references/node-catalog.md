# N8N Nodes Catalog and Python Mapping
...
| `n8n-nodes-base.scheduleTrigger` | Periodic (cron) trigger | APScheduler (`BlockingScheduler`) or systemd timer |
...
| `n8n-nodes-base.emailReadImap` | IMAP mailbox reading | `imapclient` + `email` |
| `n8n-nodes-base.executeWorkflowTrigger` | Sub-workflow called by another | Invocable Python function |
| `n8n-nodes-base.errorTrigger` | Trigger on another workflow's error | Exception hook, error decorator |

Service triggers (pattern `<service>Trigger`): map to webhooks or polling on the service's SDK (e.g.: `slackTrigger` becomes a Slack webhook or polling via `slack-sdk`).
...
| `n8n-nodes-base.respondToWebhook` | Response to received webhook | Return of the FastAPI/Flask route |
...
| `n8n-nodes-base.merge` | Parallel branch join | List combination, `dict.update`, `pandas.concat` |
...
| Type in JSON | Meaning | Python |
|---|---|---|
| `n8n-nodes-base.set` | Sets values in fields | Assignment in dict |
...
| `n8n-nodes-base.xml` | XML parse/build | `xml.etree.ElementTree`, `lxml` |
| `n8n-nodes-base.html` | HTML parse | `beautifulsoup4`, `lxml` |
| `n8n-nodes-base.markdown` | Markdown conversion | `markdown`, `mistune` |
| `n8n-nodes-base.spreadsheetFile` | XLSX/CSV read/write | `openpyxl`, `pandas`, `csv` |
...
| `n8n-nodes-base.code` | JS or Python code (newer version) | Pure Python function. Check `parameters.language` |

When translating Function/Code nodes to `design.md`: describe the logic in pseudocode, do not copy literal JS. The equivalent Python should be idiomatic.
...
| `n8n-nodes-base.questDb` | QuestDB | `psycopg` (PG wire compatible) |
...
For new projects, consider whether LangChain/LangGraph is needed or if a direct LLM SDK call is simpler.
...
Mapping N8N credential types to Python patterns:
...
| `httpBasicAuth` | Basic Auth | `(user, pass)` tuple in `httpx`, from env vars |
...
|---|---|
| Webhook | FastAPI (async, Pydantic validation) |
...
| HTTP request timeout | `httpx.Timeout(...)` explicit |
...
- Node versions (`typeVersion`) may change internal parameters. Verify that the `parameters` structure matches the version.
