...
description: Measures the active feature's structural size by reading requirements, doubts, plan, and tasks from the forward cycle, and generates size.json and size.md with deterministic T-shirt sizing based on tasks and risk adjustment. Use when the user types "/reversa-pricing-size", "reversa-pricing-size", "dimensionar feature" or "calcular tamanho da feature". Runs after `/reversa-to-do` and before `/reversa-pricing-estimate`.
license: MIT
...
4. Tolerate customized templates
5. Do not use dashes in any text
6. Every disk write is atomic, with tempfile plus rename, UTF-8 without BOM
...
Doubts can be missing without blocking. Requirements, plan, and tasks block.
...
3. Priority if multiple types: `test > infra > delete > modify > new`
...
- XL: "Very large feature, high complexity. Consider splitting into sub-features."
- XXL: "Huge feature, extreme complexity. Recommend dividing before proceeding."

Add when applicable:

- risk from high doubts
...
```
Sizing feature: <feature-dir-relative>
...
3. Path of the `.bak`, if there was recalculation
4. Next step:
