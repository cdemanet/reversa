...
When the user invokes `/reversa-migrate --auto`, the orchestrator skips human pauses and applies these defaults. Before starting, the warning to the user lists each one. Each auto-applied item is recorded in `ambiguity_log.md` with tag `auto-decided` for later review.
...
- 🟡 INFERRED items → MIGRAR (with note "validate in the coding agent").
...
- `critical` risks that would depend on a human owner stay with `owner = "to be defined"` in `risk_register.md`.

## Designer
- **Topology (Phase 1)**: accepts the proposed modern topology (option 2). Justification recorded in `topology_decision.md` is the Designer's own; in `ambiguity_log.md` stays the `auto-decided` tag for later review. Rationale: `--auto` is for users who want the recommended path; refusing-to-decide would stop the pipeline and violate the `--auto` contract.
...
- **Generation (Phase 2)**: accepts the generated `target_screens.md` and propagates deviations as `pending`. `--auto` does not approve deviations alone; they stay in `ambiguity_log.md` as `auto-decided` for later review, without blocking the handoff (exception to `--auto`: if a deviation is `type=correction` in literal mode, the agent refuses and asks for human approval even in `--auto`, because changing text without approval breaks expectation).
...
- **Missing Discovery prerequisites** (`_reversa_sdd/design-system/` or `_reversa_sdd/ui/inventory.md`): creates a minimum `tokens-derived.md` and builds the inventory only from source code; alerts in `ambiguity_log.md`.
...
> - Screen Translator (generation): deviations stay pending in `ambiguity_log.md` (not approved). Golden file capture not automated (only manifest).
