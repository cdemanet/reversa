...
3. **`screen_modernization_decision.md`**, non-negotiable reading when the legacy has UI. The chosen mode (literal / modernized / hybrid) defines how the coder will materialize the screens.

## Recommended reading order

1. `paradigm_decision.md` (mandatory, first)
2. `topology_decision.md` (mandatory, second)
3. `screen_modernization_decision.md` (mandatory when there is UI; skip if Screen Translator ran in skipped mode)
...
13. `screen_deviation_log.md` (consultive, when there is UI)
14. `risk_register.md` + `cutover_plan.md`
15. `discard_log.md` (consultive)
16. `ambiguity_log.md` (consultive)
...
> Items that need a human decision before the coding agent can start.
...
1. **Read `paradigm_decision.md` and internalize**: the target paradigm is <from paradigm_decision>. Every code choice must honor that paradigm.
2. **Read `topology_decision.md` and internalize**: the chosen topology is <preserve | modernize | hybrid>. Use the folder tree sketch recorded in that artifact as the base to create the new repository's folder structure.
3. **Read `screen_modernization_decision.md` and internalize** (when there is UI): the screen translation mode is <literal | modernized | hybrid>. In literal, materialize byte-by-byte (or pixel-equivalent) what is in `target_screens.md`; in modernized, honor the component hierarchy, tokens, and the 4 states (idle, loading, error, success).
4. **Configure the new repository** with the stack declared in `migration_brief.md` and the decided topology.
...
6. **Implement the screens** consuming `target_screens.md` as a literal contract. In literal mode with golden files present in `_reversa_sdd/screens/golden/`, the implementation output must match the golden file within the `normalizationRules` declared in `manifest.yaml`.
...
8. **For each component**, validate that it respects the chosen paradigm (explicit signals in `target_architecture.md § Honoring the chosen paradigm`) and the chosen topology (explicit signals in `target_architecture.md § Honoring the chosen topology`).
9. **For data migration**, follow `data_migration_plan.md`.
10. **For cutover**, follow `cutover_plan.md` and the go/no-go criteria.

## Auto-decided items (only if executed in --auto)
> List here items whose default was applied without human confirmation. Recommend reviewing before cutover.

- <or: pipeline executed in interactive mode, no auto-decided items>

## Final notes
<Observations from the orchestrator to the coding agent.>
