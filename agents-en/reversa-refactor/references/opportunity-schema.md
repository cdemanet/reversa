...
display_number: <n>                  # global human-friendly alias, max existing + 1
...
roi:
  confidence: green | yellow | red    # 🟢 covered and understood | 🟡 partial | 🔴 without proof
  impact: <why it pays off: hotpath, coupling, risk, clarity>
...
<opportunity description, with the observed before and the proposed transformation>
```
...
- Confirmed `soul` and `specs` that touch the target are always consulted. A confirmed business rule is never broken or treated as dead code.
- States are monotonic in the audit sense: `declined` and `reverted` preserve the history, never erase the record.
- `prune` only marks `state: applied` with `preservation.method: death-proof` and the attached proof. A suspect orphan stays `proposed` with `promoted_to: null`.
