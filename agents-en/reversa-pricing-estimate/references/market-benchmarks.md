...
**Table version:** 2.0
**Reference date for benchmarks:** 2026-05

Documents the static references that the `reversa-pricing-estimate` agent uses for the Market Range scenario. V2 removes made-up total ranges per combination and starts using hourly benchmark by country and seniority, deriving the total from the hour range in `effort-formula.md`.

## Mandatory disclaimer

The numbers are didactic approximations based on public and commercial sources known in May 2026. They do not replace updated regional research. Many public sources bring monthly or annual salary, not direct freelance rate. When the line derives a freelance rate from salary, the `source_kind` field must state this explicitly.
...
- `escopo_fechado`, `sprint`, `valor_fixo_por_entrega`: show total per feature derived by hours
- `retainer`: show "equivalent range per feature within retainer"

`client_profile` does not change the number in v2. Without a dataset per profile, multipliers by client would be invention. The estimate.md can add a qualitative alert for microempresa, pequena empresa, or enterprise.
...
- Computrabajo Mexico, Developer and Desarrollador IT salaries, updated in 2025: https://mx.computrabajo.com/salarios/desarrollador-it

## Fallback rules

1. If `country` is not in the table, Market becomes `unavailable: true`
2. If `seniority` comes as an alias, normalize and calculate
3. If `pricing_model` is not among the known models, use the `escopo_fechado` presentation and record the fallback
4. `client_profile` does not change the price in v2
5. `complexity_class` must always exist in size; if absent, fail with a message asking to recalculate the Sizer

## Countries not covered in v2

For `country` outside `[BR, US, PT, MX]`, the Market scenario becomes unavailable with explanation:

"Market range for `<country>` is not yet documented in this Reversa version. Covered in v2: BR, US, PT, MX."
...
3. Cite source and year per line
4. Do not add multipliers by `client_profile` without a dataset
5. Bump in `market` formula_version
