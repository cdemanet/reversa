...
Documents the deterministic calculation that the `reversa-pricing-estimate` agent applies for the Effort scenario. The v2 formula removes the old linear conversion of score to hours and uses hour ranges per T-shirt size, with a seniority factor inspired by the personnel capacity multipliers of COCOMO II.

## Source and criterion

COCOMO II is a parametric model of effort estimation that uses size, product attributes, platform, personnel, and project. For the Reversa UX, using the full model would be too complex. V2 only uses the defensible idea of personnel capacity multipliers, keeping simple hour ranges per class.

Main reference:

- Barry Boehm et al., *Software Cost Estimation with COCOMO II*, Prentice Hall, 2000
- Carnegie Mellon SEI, software cost estimation and COCOMO II overview: https://insights.sei.cmu.edu/blog/software-cost-estimation-explained/
...
hours_by_complexity_class_senior:
  ...
  L:   32 to 80 hours
  XL:  80 to 160 hours
  XXL: 160 to 320 hours, with mandatory scope-splitting recommendation
```
...
The `hours_estimated` field is the midpoint for compatibility and summary. The `hours_min` to `hours_max` range should be displayed in estimate.md.
...
If the profile indicates that the factor includes VAT, IVA, or highlighted tax on the invoice, estimate.md must warn that this value can be passed through to the client and does not necessarily reduce margin.

## Step 6: project markup

The historical `margin_percent` field should be treated as **project markup over direct cost**, not as net accounting margin.
...
price_total = round_currency(direct_cost + approximate_tax + applied_markup)
```

`price_total` is the midpoint of the range and exists for compatibility. The estimate.md must highlight `price_minimum` to `price_maximum`.
...
```
value_billing = round_currency(value_local / exchange_rate_to_local)
```

The estimate.md must print the rate used:
...
2. XXL remains computable, but must generate a strong scope-splitting recommendation
