...
Documents the deterministic calculation that the `reversa-pricing-estimate` agent applies for the Value scenario. The v2 formula replaces the fixed multiple of 6 to 12 months with percentage capture of the declared annual economic value.

## Source and criterion

Value-based pricing uses the value perceived or economic value to the client as the price basis, not just internal cost or competitors' price.
...
The 10% to 30% range is a commercial heuristic from Reversa for B2B/freelance/agency. It should be described as capturing part of the annual value, not as a universal academic law.
...
if monthly_return_declared == 0 AND cost_of_not_doing == 0:
  available = false
  explanation_en = "Value scenario cannot be calculated: client did not declare measurable return."
```
...
The client can declare:

- recurring monthly return
- annual cost of not doing
- both

When both exist, the formula uses the largest defensible economic value.
...
2. The 10% to 30% capture range is documented heuristic
3. `users_impacted` does not enter the v2 numerical calculation
4. Extreme values are not truncated
5. The explanation can mention payback months, but should not say the price is "6 to 12 months"
