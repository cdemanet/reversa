...
Table of palettes used by the seal, derived from the visual style chosen by the user of `/reversa-documentation`.
...
- `fg`: color of the label text outside the canvas.
...
Visual translation:
- Background: broken paper.
- Foreground: petroleum blue, stone gray, terracotta, moss green, sand.
- Accent: deep midnight blue.

**Dark variant**: for use as mini-seal on dark header, mirror (bg ↔ fg).
...
Visual translation:
- Background: blueish night black.
...
- Accent: blueish black.

**Typical use**: system documentation with many components to distinguish; seal covers multiple hues.
...
**Typical use**: documentation with strong presence of 3D scenes; the seal dialogues with the `arquitetura.html` aesthetic.

---

## Palette `other` (fallback)

When the user chooses "Other" in the style menu and provides a free description, the skill maps the description to the closest palette, or applies a basic heuristic:

```javascript
function paletteFromFreeform(text) {
    const lower = text.toLowerCase();
    if (/(luxur|premium|cinematog|dark)/.test(lower)) return palettes.premium;
    if (/(techn|sober|clean|minimal)/.test(lower)) return palettes.sober;
    if (/(dense|saturat|colorful|vibra)/.test(lower)) return palettes.dense;
    if (/(explora|3d|luminous|etherea)/.test(lower)) return palettes.exploratory;
...
Even with 5 colors in `foreground`, the seal does not use all equally. Visual proportion rule:
...
Patterns like `crystal-lattice` use colors in distinct layers, but the most visible layers (outer) use colors 1 and 2; inner layers use 3, 4, 5.

---

## Automatic adaptation for mini-seal

In mini-seals (<200px), the palette is simplified to 3 colors only:
...
Before rendering, verify that `accent` has enough contrast with `bg` for patterns that highlight the center (minimum 4.5:1 ratio per WCAG AA).
...
If contrast fails, substitute `accent` with a lighter/darker version derived automatically.
