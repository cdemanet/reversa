...
Fallback: minimum SVG (circle + palette background color) inline, without p5 dependency.

---

## ERR-02: Canvas not supported by the browser

**Cause**: very old browser without `<canvas>` support (very rare today).

**Detection**: `canvas.getContext("2d")` returns `null`.

**Handling**: fall back to inline SVG with `crystal-lattice` (which is the most compatible pattern with real SVG).

---

## ERR-03: Invalid or absent seed

**Cause**: agent called the skill without seed, or passed an empty string.

**Detection**: validation at entry.
...
function resolveSeed(rawSeed) {
    if (!rawSeed || typeof rawSeed !== "string" || rawSeed.length === 0) {
        ...
        console.warn("Absent seed, using timestamp as fallback. Seal will not be reproducible.");
        return timestamp;
    }
    return rawSeed;
}
...
When timestamp is used, show a warning in the page footer (only if it is a large hero): "Non-reproducible seal (no seed)".
...
**Cause**: palette with `accent` and `bg` too close, generating invisible central element.
...
**Cause**: heavy pattern in small canvas consuming disproportionate CPU.

**Detection**: measure time between final `setup` and `draw`.

**Handling**: if canvas is mini (<200px) and chosen pattern is `wave-interference` (pixel loop), switch automatically to `crystal-lattice` (simple geometry) with a console message.

---

## ERR-09: Multiple instances of the same seal on the same page

**Cause**: the mini-seal appears on all mini-site pages. Reloading p5.js and generating a canvas on each is wasteful.

**Handling**: generate the seal once as SVG (for `crystal-lattice`) or PNG dataURI (for other patterns) and embed inline in all pages. The skill accepts the `mode: "svg" | "dataURI" | "html"` parameter to return the appropriate format.
...
## ERR-10: Corrupted localStorage of seed
...
If the invoker lost the seed, the agent must recalculate from soul.md (sha256). This skill is not responsible for that.

---

## General principle

The seal is a **decorative** element. Seal failure must never break the whole page. In all the above scenarios, there is a fallback that always renders something: a colored circle, a minimum SVG, a simplified version. No white screen.

Messages in en, no dashes.
