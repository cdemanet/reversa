...
**Cause**: user is offline on the first run, or CDN blocked by corporate firewall.

**Detection**: the `<script type="module">` script fails to import, or `THREE` stays `undefined` after loading.
...
Fallback displays a static version of the scene (pre-rendered screenshot if any, or symbolic ASCII art) with a clear message.
...
Non-fatal errors (individual bad module) discard the item with a warning. Fatal errors (invalid root structure) show a clear message.
...
**Detection**: after `loadData()`, count of items.
...
Friendly empty state, never a silent empty scene.
...
**Handling**: apply grouping automatically and warn.
...
**Cause**: graph with a closed cycle and iterative layout without a stopping criterion.

**Detection**: measure simulation iterations; if it passes `MAX_SIM_FRAMES` without converging, abort.

**Handling**: stop the simulation at the limit frame, show warning "Layout did not converge, positions may not reflect ideal stability", draw anyway.

---

## ERR-09: WebGL context lost

**Cause**: inactive tab for a long time, graphics driver change, overloaded GPU.
...
Instead of reloading the page, rebuild the scene on the same canvas. It's important to call `rebuildScene()` which recreates textures and buffers.

---

## ERR-10: Corrupted localStorage sidebar

**Cause**: old localStorage data with incompatible format after a skill update.

**Detection**: `JSON.parse` fails when restoring state, or value is outside the expected slider range.
...
`reversa-error-panel`, `reversa-warning-banner`, `reversa-toast` styles live in the mini-site shared CSS.

---

## General principle

No error should result in a **silent white screen**. Always show a clear message in English with actionable instruction or clear indication of limitation. Short messages, no framework jargon.
