...
Animated camera going through the scene at a cinematic pace, with synchronized **narrative overlay**. Works as a "trailer" of the system: someone presses play and the video unfolds by itself, stopping at key points with explanatory captions.
...
- Onboarding of new devs ("press play and see how the system is").
- Short executive demo (1 to 3 minutes).
- Companion to the mini-site's `deck.html`.
...
- `at`: second in the timeline at which the waypoint fires.
- `camera`: camera position and target on arrival.
- `highlight`: list of node/module IDs to highlight (others defocus).
- `overlay`: caption text, in English.
...
During highlights, the selected modules gain emissive and the others reduce opacity.
...
- End showing the layer stack of the violating layers (if any).

Each heuristic can be turned on or off via parameter.
...
Tour inherits the performance of the base mode. Adding the tour costs little: only camera interpolation and opacity animations. Be careful with `switchMode` mid-tour: dispose + rebuild can cause 200-500ms stutter.
