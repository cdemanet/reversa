...
Oriented dependency graph between modules visualized in 3D, with force simulation (attraction between connected nodes, repulsion between unconnected ones) that distributes the graph in space organically.
...
- Detect **high coupling** (very connected nodes stay in the center of the cluster).
- Identify **central modules** (high fan-in) and **isolated modules**.
- Visualize **dependency cycles** (visible loops in the simulation).
- Compare cohesion between folders (modules in the same folder should be close).

**When to avoid**: projects with more than ~300 modules, where the graph becomes an illegible hairball. Use Code City or group by folder.

## Layout algorithm: 3D force

D3-force-like simulation adapted for 3 dimensions. Runs in a loop until it stabilizes, then freezes.
...
Use `InstancedMesh` of spheres for up to 1,000 nodes; above that, billboard with sprites.
...
Curved lines in 3D using `BufferGeometry` with `LineSegments` or `TubeGeometry` for edges with volume.
...
For oriented edges with visible arrow, use `ArrowHelper` or small cones close to the target.
...
Run Tarjan or Kosaraju before rendering; nodes that belong to cycles receive a special color (orange) and their edges become red.
...
Slider changes re-activate the simulation for 100 more frames before freezing again.

## Interaction

- **Hover on node**: tooltip with name, number of dependents (fan-in), dependencies (fan-out).
- **Click on node**: highlights the node and its connected edges, defocuses the others (reduced opacity).
...
Edges: up to **10,000** with LineSegments. Above that, simplify (show only top N by weight) or use a color gradient instead of duplicating geometry.
