...
Tree (or DAG) of **function calls** explorable in 3D. Each node is a function, each edge is a call. Lets you navigate the depth of an invocation chain starting from entry points (endpoints, handlers, main).
...
**When to avoid**: static analysis without execution data is incomplete (does not capture polymorphism). For a structural view use Dependency Graph 3D.
...
Each entrypoint becomes the root of the tree. Depth increases on the Z axis (moving away from the camera), functions at the same level are distributed in an XY plane.
...
For multiple entrypoints, each one occupies a region of the XY plane (translation at center), creating parallel trees.
...
Curved bezier-style lines connecting parent to child. Thicker for high `weight`.
...
- Above that, collapse subtrees automatically after depth 5 and show "+N functions" button.
