...
Well-established pattern of 3D software visualization: each project file is a **building**, grouped in **districts** that correspond to folders. Lets you grasp the size, complexity and distribution of the code at a single glance.
...
| File folder | District (plane position) |
...
- Detect folder grouping (cohesive vs scattered districts).
...
See `THREE_PATTERNS.md` for the InstancedMesh pattern. Each building is an instance of the same BoxGeometry, with different matrix and color.
...
Add a large plane as the ground and colored squares delimiting each district.
...
- **Click on building**: focuses the camera on the building (animates `controls.target` to the building position).
- **Drag on district**: rotates the camera with OrbitControls.
- **Scroll**: zoom in/out.

## Performance

- Up to **5,000 buildings** is safe with InstancedMesh.
- Above that, group files by folder (one building = one folder with aggregated LOC height, area by file count).
- Disable shadows if the framerate drops below 30fps (detect via `requestAnimationFrame` timer).

## Optional variants

- **Temporal Code City**: animate growth along the project history (each commit makes buildings grow).
- **Code City colored by author**: colors indicate who is the primary maintainer of each file.
- **Code City with rain**: hot paths receive a red falling particle effect, indicating "instability".

These variants are for future skill versions.
