...
| Module within the layer | Box/disc positioned on the layer plane |
| Inter-layer dependency | Oriented vertical line connecting modules |
| Flow direction | Arrow at the tip of the line |
| Layer violation (lower layer importing from upper) | Pulsing red line |
...
The skill accepts the layer mapping coming from the user (via JSON) or tries to infer from folder patterns.
...
Simple grid packing in 2D on the layer plane.
...
edges.forEach((e) => {
    ...
    if (isViolation) line.userData.pulse = true; // animates opacity
    scene.add(line);
});
```
...
The default rule (Clean Architecture): layers only depend on layers with higher `order` (more "inward").
...
- **Click on violation**: focuses the camera on the two modules involved and shows details of the relation in the panel.
