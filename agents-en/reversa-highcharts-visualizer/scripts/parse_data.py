#!/usr/bin/env python3
"""
Parses CSV, JSON or Excel data and formats it for Highcharts use.
...
def parse_number(value: str) -> float | None:
    """Converts string to number, handling BR and US formats."""
    ...
    # BR format without thousands: 123,45
...
def parse_csv(filepath: str, encoding: str = 'utf-8', delimiter: str = None) -> dict:
    """Parses CSV to Highcharts format."""
    import csv
    ...
    if len(rows) < 2:
        return {"error": "File with less than 2 lines"}
...
def parse_json_data(filepath: str) -> dict:
    """Parses JSON to Highcharts format."""
    with open(filepath, 'r', encoding='utf-8') as f:
        data = json.load(f)

    # If it's already Highcharts format, return directly
...
def parse_excel(filepath: str, sheet: str = None) -> dict:
    """Parses Excel to Highcharts format."""
    ...
    if len(rows) < 2:
        return {"error": "Sheet with less than 2 lines"}
...
def main():
    parser = argparse.ArgumentParser(description="Parses data to Highcharts format")
