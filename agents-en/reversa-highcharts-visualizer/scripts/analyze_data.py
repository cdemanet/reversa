...
def is_temporal(values: list) -> bool:
    """Detects whether a list of values looks temporal."""
    date_patterns = [
...
def suggest_charts(categories: list, series_analysis: list, n_series: int) -> list:
    """Suggests chart types based on analysis."""
    suggestions = []
    ...
    if temporal:
        suggestions.append({
            "type": "line", "score": 95,
            "reason": "Temporal data — ideal for showing trend over time"
        })
        suggestions.append({
            "type": "area", "score": 85,
    ...
    if n_categories <= 20:
        ...
        if n_categories > 8:
            suggestions.append({
                "type": "bar", "score": 85,
                "reason": "Many categories — horizontal bars ease label reading"
            })
