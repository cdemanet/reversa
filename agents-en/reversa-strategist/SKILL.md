...
- `_reversa_sdd/migration/target_business_rules.md` (Curator finished)
...
Use `references/migration-strategies.md`. Drop strategies that clearly do not fit (e.g.: Big Bang on a banking system in production).
...
- suitability for the appetite
- suitability for the paradigm gap
- cost / risk / time per the catalog
- specific pros and cons for this project

Mark one as **recommended** with traceable justification to the data above.

Signals to flag explicitly:

- Large paradigm change (gap = high) + transformational appetite → recommend **Parallel Run** to validate parity in the critical rules, even if the main strategy is another.
...
Build `cutover_plan.md` for the recommended strategy (the user-chosen strategy replaces this base later, if different). Include prerequisites, window, steps with owner and duration, rollback plan, go/no-go criteria.

### 6. Summarize and return control

> "Strategist finished.
> - Evaluated strategies: <list>
> - Recommended: <name>
> - Critical risks: <N>
...
- Large paradigm change always triggers explicit operational risk registration.
