...
- **Type**:
  - `technical`: target platform limitation (e.g.: Windows terminal without UTF-8 without `chcp 65201`).
  ...
  - `correction`: legacy visual bug that the target fixes (e.g.: typo in label).
- **Approval**: `pending` | `approved` | `rejected`.
- `approved` deviation → also listed in `parity_specs.md § Exceptions`.
- `pending` deviation → blocks handoff to the Inspector.
- `rejected` deviation → archived with explicit note; agent regenerates the screen in conformant mode.
...
<General observations about the deviation set: patterns, learnings that apply to future migrations in the same source→target pair, suggestions for improved adapter for v2.>
