...
Heuristics that `reversa-screen-translator` uses to classify the source platform of the legacy from the content of `_reversa_sdd/inventory.md` and the source code. Use together with `references/adapter-pairs.md` to choose the adapter.
...
- 🔴 **GAP**: source code artifact absent; classifies only by `inventory.md`.
...
| `delphi-vcl` | `.pas`, `.dfm`, `.dpr` | `unit `, `interface`, `TForm`, `TPanel`, `TButton` in `.dfm` | `.pas` pure without `.dfm` (likely CLI) |
| `delphi-firemonkey` | `.pas`, `.fmx` | `TForm` in `.fmx` file (FireMonkey) | only `.pas` |
| `vb6` | `.frm`, `.bas`, `.cls`, `.vbp` | `VERSION 5.00` in header, `Begin VB.Form`, `Begin VB.CommandButton` | `.bas` pure (module without UI) |
...
| `php-server-rendered` | `.php` | `<?php ... ?>` + HTML inline + `mysql_*` or `mysqli_*` | only `.php` in `api/` folder (likely REST API, not UI) |
| `html-legacy-jquery` | `.html`, `.htm`, `.js` | `jQuery`/`$.ajax` + form submits server-side, without SPA framework | static HTML (without dynamic JS) |
...
- **Android Java vs Kotlin**: `.java` + `.kt` in the same project → migrating project; classify per individual file.
- **iOS Storyboard vs XIB**: both supported; treat as one class (`ios-xib-*`). Difference is in the capture detail.
