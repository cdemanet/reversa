...
Usage:
    ...
    python scripts/spec_scorer.py --spec spec.md --json   # output as JSON
"""
...
def section_content(text: str, section_pattern: str) -> str:
    """Extracts section content until the next same-level section."""
...
def has_numeric_metric(text: str) -> bool:
    ...
    return bool(re.search(r'[<>≤≥]\s*\d+|=\s*\d+\s*%|\d+\s*(ms|min|h|%|days?|days?)', text))


def has_vague_terms(text: str) -> list[str]:
    vague = ["quickly", "soon", "briefly", "some", "many",
             "efficiently", "intuitively", "easy to use",
             "user-friendly", "performant", "look nice", "be fast",
             "be good", "rapidamente", "logo", "brevemente", "alguns", "muitos",
...
def has_contradictions_signal(text: str) -> bool:
    ...
    return bool(re.search(r'RF-\d+.*?\b(but|however|yet|mas|porém|entretanto)\b', text,
...
def score_completeness(text: str) -> DimensionScore:
    ...
    required_sections = [
        (r'^#{1,2}\s+1[\.\s]+Sumary', "Section 1 (Summary)"),
    ...
    if rf_count >= 5:
        ...
        score += 2
        dim.issues.append(f"❌ Few RF requirements ({rf_count}) — spec incomplete")
    else:
        dim.issues.append("❌ No numbered RF requirements — impossible to track")
    ...
    if ng_count >= 3:
        ...
        dim.issues.append("❌ Non-goals absent — risk of scope creep")
    ...
    if placeholders == 0:
        ...
        dim.issues.append(f"❌ {placeholders} unfilled placeholder(s) — spec incomplete")
...
def score_testability(text: str) -> DimensionScore:
    ...
    if has_happy_path:
        # Verify there are at least 3 numbered steps
    ...
    if has_numeric_metric(goals_content) or has_numeric_metric(text[:2000]):
        ...
        dim.issues.append("⚠️ Success metrics without numerical values — hard to validate")
...
def score_clarity(text: str) -> DimensionScore:
    ...
    # Open questions signaled
    open_questions = count_pattern(text, r'⚠️\s*OPEN:|OQ-\d+')
    ambiguities_hidden = count_pattern(text, r'\?.*\?')  # multiple question marks — signal of doubt
    if open_questions > 0:
        score += 6
        dim.positives.append(f"✅ {open_questions} open question(s) explicitly signaled")
    elif ambiguities_hidden > 3:
        dim.issues.append("⚠️ Possible ambiguities not signaled (use ⚠️ OPEN: or section 14)")

    # Clear subject in requirements
    rf_section = section_content(text, r'^#{1,2}\s+6[\.\s]+Requirements')
    subjects = count_pattern(rf_section, r'\b(the system|the user|the platform|o sistema|o usuário|a plataforma)\b')
    ...
    # Technical language without definition (heuristic)
    vague = has_vague_terms(text)
    if not vague:
        score += 8
...
def score_scope(text: str) -> DimensionScore:
    ...
    # Mapped dependencies
    has_deps = has_section(text, r'10[\.\s]+Integra|Dependencies')
    if has_deps:
        deps_content = section_content(text, r'10[\.\s]+Integra|Dependencies')
    ...
    # Rollout plan
    has_rollout = has_section(text, r'Rollout|Launch Plan|13[\.\s]+')
    if has_rollout:
        score += 3
...
def score_edge_cases(text: str) -> DimensionScore:
    ...
    if ec_count == 0:
        dim.issues.append("❌ CRITICAL: No edge case defined — implementation will not know how to handle errors")
        dim.raw_score = 0
        return dim
    ...
    if has_behavior >= ec_count * 2:  # at least trigger + behavior
        score += 3
        dim.positives.append("✅ Edge cases with expected behavior defined")
    else:
    ...
    covers_external = bool(re.search(
        r'(timeout|unavailable|down|failure|error\s+\d{3}|retry|fallback|indisponível|fora do ar|falha)',
...
def classify(score: float) -> str:
    ...
    return "❌ Insufficient — Back to interview/draft"
...
def print_report(report: SpecReport):
    ...
    positives = [p for d in dims for p in d.positives]
    if positives:
        print(f"\n  ✅ STRENGTHS:")
...
def main():
    ...
    # Non-zero exit code if score < 65 (for CI/CD use)
