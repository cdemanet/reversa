...
Skill to build structured JSON prompts for product image generation with a cinematic,
luxurious aesthetic — optimized for **Nano Banana 2 (Gemini 3.1 Flash Image)** via
**Google Antigravity**, with support for all the model's native parameters.
...
2. **Confirmation** — Show summary and ask for approval
3. **JSON generation** — Build the final structured prompt

---

## STEP 1 — Guided interview in blocks
...
1. **Product type**: What is the product? (e.g.: chocolate cake, perfume bottle, sneaker, shake, jewelry...)
...
6. **Main action**: Is the product static or in motion? (e.g.: liquid exploding, suspended particles, smoke, splash, slice revealing the interior...)
7. **Suspended elements**: Which elements fly around the product? (e.g.: drops, dust, fragments, leaves, crystals, bubbles...)
...
After collecting all answers, show a **bulleted summary** for the user to confirm:
...
```json
{
  "master_prompt": {
    ...
    "product": {
      "type": "[rich, adjectivated product description]",
    ...
    "composition": {
      ...
      "placement": "[centered hero placement on the specified surface]"
    },
    "lighting": {
    ...
    "color_palette": {
      "background": "[gradient/bokeh of the background with transition description]",
...
- `output_specs.aspect_ratio` must use the model's native supported values
...
> 💡 **Usage tip for Antigravity:** Paste this JSON directly into the Nano Banana 2 prompt field in Google Antigravity. The `output_specs` fields are interpreted natively by the model — no additional prefix is needed.
...
For inspiration on language patterns, see `/mnt/skills/user/image-prompt-builder/references/examples.md` if available.
