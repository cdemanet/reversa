...
1. Read `.reversa/active-requirements.json`
   1.1. If missing, abort pointing to `/reversa-requirements`
2. Verify the existence of `feature-dir/roadmap.md`
   2.1. If missing, abort with clear message pointing to `/reversa-plan`. Do not try to fill the roadmap here
3. Also load `feature-dir/data-delta.md` and `feature-dir/interfaces/*` if they exist
4. Apply `before-to-do` in the default way
...
1. Use the five standard phases in order:
   ...
   1.3. Core (core feature logic)
   1.4. Integration (gluing with other parts of the system, external contracts, hooks)
   1.5. Polish (logs, telemetry, messages, short documentation)
2. For each item in `roadmap.md`, derive one or more actions
3. Break each action to the point where it can be executed in a single coherent block, without needing to switch context
...
- If an action has more than five logical subpoints, break it
...
2. For each phase, create table with columns `ID | Description | Dependencies | Parallelism | Target File | Confidence | Status`
...
Apply `after-to-do` in the default way.
...
3. Total marked as `[//]`
4. Next step suggestion, in order:
